...
class Substract:
    ...
    def ping(self, data: dict):
        print("The substract agent receives a ping from the Yellow Page.\n " + str(data))
    ...
    def receive_list_agents(self, data: dict):
        print("The substract agent receives from the Yellow Page the agent directory in encrypted form.\n " + str(data))
        data_desencrypted = self.management_security.decrypt_data(data)
    ...
    def execute(self, data: dict):
        print("The substract agent receives a request to perform a substract operation.\n " + str(data))
        data_desencrypted = self.management_security.decrypt_data(data)
        print("The request is decrypted using hybrid encryption: \n" + str(data_desencrypted))
        if data_desencrypted["num1"] and data_desencrypted["num2"]:
            print("The substract agents.py has received: " + str((data_desencrypted["num1"], data_desencrypted["num2"])))
            result = data_desencrypted["num1"] - data_desencrypted["num2"]
            print("The substract agent is sending: " + str(result))
...
def execute_substract(management_security: SecurityManagement):
    substract = Substract(management_security)
    execute_agent(substract, "substract", management_security)
