...
class BaseAgent:
    ...
    def ping(self):
        ...
        self.management_security.management_logs.log_message(ComponentType.BASE_AGENT, f"{self.agent_name} -> Ping processed", LogType.END_CONNECTION, time_str=f"{end_time - start_time:.9f}")
        return "pong"
    ...
    def update_list_agents(self, encrypted_data: dict):
        ...
        try:
            ...
            self.management_security.management_logs.log_message(ComponentType.BASE_AGENT, f"{self.agent_name} -> List of agents updated successfully", LogType.END_DECRYPTION, time_str=f"{decryption_time:.9f}")
        ...
        self.management_security.management_logs.log_message(ComponentType.BASE_AGENT, f"{self.agent_name} -> Total update list agents time", LogType.END_ENCRYPTION, time_str=f"{total_time:.9f}")
    ...
    def receive_list_agents(self, encrypted_data: dict):
        ...
        end_time = time.perf_counter()
        self.management_security.management_logs.log_message(ComponentType.BASE_AGENT, f"{self.agent_name} -> Received list of agents", LogType.END_ENCRYPTION, time_str=f"{end_time - start_time:.9f}")
