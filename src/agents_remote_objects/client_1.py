...
class Client1(AgentConsumer):
    ...
    def request_service(self, service_id: str, request_data: dict):
        ...
        self.management_security.management_logs.log_message(ComponentType.CLIENT, f"{self.agent_name} -> Total request service time", LogType.END_REQUEST, time_str=f"{end_time_total - start_time_total:.9f}")
    ...
    def list_available_services(self):
        ...
        end_time_total = time.perf_counter()
        self.management_security.management_logs.log_message(ComponentType.CLIENT, f"{self.agent_name} -> Total listing services time", LogType.END_REQUEST, time_str=f"{end_time_total - start_time_total:.9f}")
