...
class ManageDataAgent:
    ...
    def save_data_conecction_agent(self, data_client: AgentType):
        ...
        if not os.path.exists('agents_remote_objects/data'):
            os.makedirs('agents_remote_objects/data')
        # Si ya existe el archivo, se sobreescribe
        with open(f'agents_remote_objects/data/{name_client}.json', 'w') as file:
            json.dump(data_client, file)

    def get_data_conection_agent(self, name_client):
        try:
            with open(f'agents_remote_objects/data/{name_client}.json', 'r') as file:
                data = json.load(file)
